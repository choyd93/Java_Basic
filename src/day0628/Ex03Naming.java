...
//          모두 대문자로 적은 경우에는 단어의 사이에 _을 넣어
//          단어를 구분시키는 방법이다.
//          예시 : MY_NUMBER, UNIT_PRICE 
//       C. 무언가 이름을 지어줄 때에는 _를 제외한 특수문자는 사용할 수 없다.
//       D. 이름의 중간이나 끝네 숫자가 올 수 있지만 숫자로 이름을 시작할 수 없다.
//       E. 한글로 이름을 지어줄 수 있지만 절.대.로 하지 말자.
//       F. 자바에서는 내부적로 다른 의미가 실행되는 50가지의 단어들이 있다.
//          해당 단어들을 우리가 예약어(keyword)라고 부르는데
//          해당 단아들은 이름이 될 수 없다.
//          그러므로 50개를 외웁시다 -> 옛날 예적 이야기..
//          이클립스에서는 해당 키워드를 자주색으로 표시해준다.
//          만약 여러분들이 어떤 이름을 지었는대 자주색으로 표시되면?
...
//    예시 : Ex03Naming, MerberContriller
// 4. 메소드
