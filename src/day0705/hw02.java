...
public class hw02 {
    ...
    public static void main(String[] args) {
        ...
        // 2) 최고 기록을 저장할 int 변수
        int bestScore = 0;
        // 3) 사용자가 입력 가능한 최소값을 저장한 int 상수
        final int NUMBER_MIN = 1;
        // 4) 사용자가 입력 가능한 최대값을 저장한 int 상수
        final int NUMBER_MAX = 100;

        // 2. 변수 배열 저장
        // 1) 점수 배열
        int[] scoreArray = new int[STUDENT_SIZE];
        ...
        while (true) {
            ...
            if (userChoice == 1) {
                ...
                // 5.사용자가 입력한 숫자와 현재 기록을 저장할 int 변수
                int userNumber = 0; // <- 배열로 만들어야 할까
                int currentScore = 1; // <- 배열로 만들어야 할까

                boolean inputSwitch = false; // <- 이게 중요한듯 함.
                int index = 0; // <- 이게 중요한듯 함.

                // 7. 학생 5명 정보 입력 받고 잘못 입력 했을 시 블록하는 코드
                while (index < STUDENT_SIZE) {

                    // 6. userNumber가 computerNumber와 다를 동안
                    // while을 반복한다.
                    ...
                    // 더이상 입력할 수 없는 상태
                    System.out.println("이미 게임을 5번 진행했습니다.");
                }
