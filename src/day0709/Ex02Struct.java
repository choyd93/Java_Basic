...
public class Ex02Struct {
    ...
    public static void main(String[] args) {
        ...
        // 단, 이때에는 각 인덱스 번 칸에 new Student(); 로 초기화를 하지 않으면
        // NullPointException 이라는 에러가 발생하게 된다.
        // Null이란?
        // 스택 영역에 해당 참조형 변수 안에 힙 영역의 주소값이 
        // 저장이 되었지만
        // 실제 값을 가지고 있어야 할 힙 영역에 해당 공간이
        // 아직 아무런 준비가 되지 않은 상태를 널(Null)이라고 한다.
        // 널인 상태의 참조형 변수 안에 필드 혹은 메소드를 접근할려고 하면 
        // 아직 준비가 안된 상태이기 떄문에 NullPointerException이라는
        // 에러가 발생하게 되는 것이다. 
