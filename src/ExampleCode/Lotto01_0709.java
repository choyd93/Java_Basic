...
public class Lotto01_0709 {
 ...
 static int[] computerNumbers = new int[SIZE];
 ...
 public static void setComputer() {
    ...
    while(ArrayUtil.size(computerNumbers) < SIZE) {
        int number = random.nextInt(NUMBER_MAX)+1;
        if(!ArrayUtil.contains(computerNumbers, number)) {
            computerNumbers = ArrayUtil.add(computerNumbers, number);
        }
    }
 } // 이거를 없에고 밑에서 메소드 바꿈
 */
...
public static int[] insertAuto() {
 ...
 public static int[] insertManual() {
     int[] arr = new int[0];
     String message = "1~45번 사이의 숫자를 입력해주세요.";
     while(ArrayUtil.size(arr) < SIZE) {
